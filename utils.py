# This python file is mainly used to load datasets
...
def get_mnist(training_percent):
    """
    Load MNIST dataset
    Params:
    - Training_percent: Set the percentage of data to be used for training
    Returns:
    - Two namedtuples, with MNIST training and testing data 
    ...
    size = np.int32(training_percent * X_train.shape[0])
    indices = np.random.randint(0, X_train.shape[0], size=size)  # Generate random row indices
...
def get_CIFAR():
    ...
    - Training_percent: Set the percentage of data to be used for training
    Returns:
    - Two namedtuples, with cifar10 training and testing data 
      trn.X is training data
      trn.y is trainiing class, with numbers from 0 to 9
      trn.Y is training class, but coded as a 10-dim vector with one entry set to 1
      similarly for tst
...
def get_IB_data(ID):
    ...
    - ID: dataset name 
    Returns:
    - Two namedtuples, with SZT training and testing data 
      trn.X is training data
      trn.y is trainiing class, 
      trn.Y is training class, but coded as a binary-data vector
      similarly for tst
    """
    # binary data
    nb_classes = 2
    # the data is stored in the given path
